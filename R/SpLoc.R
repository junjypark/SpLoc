  ...
  if (isTRUE(partition)){
    ...
    if (isTRUE(parallel)){
      ...
      result=foreach(i=1:npartition, .packages=("SpLoc"),.noexport = "SpLocC" )%dopar%{
        SpLocMeanC(NNList[[i]], ymat, nperm, alpha, seed)
      }
      stopCluster(cl)
    } else{
      result=list()
      for (i in 1:npartition){
        result[[i]]=SpLocMeanC(NNList[[i]], ymat, nperm, alpha, seed)
      }
    }
    ...
    out=SpLocMeanC(NNmatrix, ymat, nperm, alpha, seed)
...
SpLocDiff=function(NNmatrix, ymat, group, nperm=1000, alpha=0.05, seed=NULL, 
  ...
  if (isTRUE(partition)){
    ...
    if (isTRUE(parallel)){
      ...
      result=list()
      for (i in 1:npartition){
        result[[i]]=SpLocDiffC(NNList[[i]], ymat, group, nperm, alpha, seed)
      }
    }
    ...
    out=SpLocDiffC(NNmatrix, ymat, group, nperm, alpha, seed)
    out$pvalue=(1+sum(c(out$permMax)>max(out$Tstat,na.rm=TRUE)))/(1+nperm)
    out$seed=seed
    return(out)    
  }
...
# SpLocMean2=function(NNmatrix, ymat, nperm=1000, alpha=0.05, seed=NULL, 
#                    is.sparse=F,partition=F, npartition=1, parallel=F, ncores=1){
#   if (length(which(is(NNmatrix)=="sparseMatrix"))==0){
#     stop("NN is not a sparse matrix. Please refer the Matrix R package to convert it.")
#   }
...
#         SpLocMeanC(NNList[[i]], ymat, nperm, alpha, seed, pU@address)
#       }
#       stopCluster(cl)
#     } else{
#       result=list()
#       for (i in 1:npartition){
#         pU=big.matrix(nrow(NNList[[i]]), nperm, type = "double")
#         result[[i]]=SpLocMeanC(NNList[[i]], ymat, nperm, alpha, seed, pU@address)
#       }
#     }
#     
#     out=combine(result, alpha=alpha)
...
#     out=SpLocMeanC(NNmatrix, ymat, nperm, alpha, seed, pU@address)
#     out$pvalue=(1+sum(c(out$permMax)>max(out$Tstat,na.rm=TRUE)))/(1+nperm)
#     out$seed=seed
#     return(out)    
#   }
...
# 
# 
# SpLocDiff2=function(NNmatrix, ymat, group, nperm=1000, alpha=0.05, seed=NULL, 
#                    is.sparse=F,partition=F, npartition=1, parallel=F, ncores=1){
#   if (!all.equal(sort(unique(group)),c(-1,1))){
#     stop("group should have either 1 or -1.")
#   }
